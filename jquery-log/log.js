...
$('#log-start-btn').click(start_log)
$('.log-filter-level').click(update_log_filter)


function start_log() {
    for (let i of example_logs) {
        for (let j of i) {
            let el = format_log(j['time'], j['level'], j['msg'])
...
function format_log(timestamp, level, msg) {
...
function update_log_filter(event) {
    let id = this.id.slice(15)
    $('#' + this.id).toggleClass('selected-log-filter')
